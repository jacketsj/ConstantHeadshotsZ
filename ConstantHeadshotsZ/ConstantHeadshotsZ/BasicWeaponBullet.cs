...
{
    public class BasicWeaponBullet
    {
        ...
        public BasicWeaponBullet(ContentManager Content, Vector2 newVector)
        {
            sprite = new Sprite(Content.Load<Texture2D>("BasicWeaponBulletSmall"), newVector);
            this.Content = Content;
            origin = new Vector2(-22, 50);
        ...
        public BasicWeaponBullet(ContentManager Content, Vector2 newVector, float newRotation)
        {
            sprite = new Sprite(Content.Load<Texture2D>("BasicWeaponBulletSmall"), newVector);
        ...
        public bool Update(Level level, Player player)
        {
            ...
            if (CollidesWithSolids(level.solids) || collidedWithZombies || isOutOfBounds(level))
            {
                return true;
            }
            return false;
        ...
        private bool CollidesWithZombies(Level level)
        {
            ...
            for (int i = 0; i < level.zombies.Length; i++)
            {
                if (CollidesWithZombie(level.zombies[i], collisionRec))
                {
                    ...
                    level.bloodParticles = level.GenerateBurst(level.bloodParticles, CHZ.options.bldCol, new Vector3(level.zombies[i].sprite.vector, 0), Content, 60, 360, 600, 1100, 0.1f, 2.7f);
                    /*
