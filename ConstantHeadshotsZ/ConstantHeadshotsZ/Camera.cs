...
namespace ConstantHeadshotsZ
{
    public class Camera
    {
        ...
        {
            transform = Matrix.CreateTranslation(new Vector3(-position.X, -position.Y, 0)) * Matrix.CreateRotationZ(rotation);// *Matrix.CreateTranslation(new Vector3(graphicsDevice.Viewport.Width * 0.5f, graphicsDevice.Viewport.Height * 0.5f, 0));
            //transform *= Matrix.CreateRotationX(MathHelper.PiOver4 / 8);
