...
namespace ConstantHeadshotsZMP
{
    public class _3DView
    {
        ...
        public void Draw(GraphicsDevice device, Level level, Player[] players, bool inTwoPlayer, int playerNo, ContentManager Content, Player player)
        {
            ...
            if (inTwoPlayer)
            {
                ...
                {
                    _3DSprite sprite = new _3DSprite();
                    sprite.Texture = player2.sprite.getTexture();
                    sprite.Position = new Vector3(player2.sprite.vector.X, 0, player2.sprite.vector.Y);
                    sprite.Draw(quadDrawer, cameraPosition, view, projection);
                }
            }
            else
